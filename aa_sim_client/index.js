...
function groupUpdate() {
	ioUser.emit('updateUserListLoc', clientMarkerUpdate);
}


function pairInit() {
	ioUserNew = ioClient.connect("http://127.0.0.1:8080/random", {
												forceNew: false,
											});
	ioUserNew.on('connect', function(){
		console.log('[User Pair] Connected');
	}); 	
}

function pairMatching() {
	ioUserNew.emit('initPairUser');
		// pair events
	ioUserNew.on('updateDriverLoc', function(data) {
		console.log('[Target Driver LocChanged]:');
		console.log(data);
	});

	ioUserNew.on('updatePetStatus', function(data) {
		console.log('[Target Pet Status Changed]:');
		console.log(data);
	});
}

function pairUpdate() {
	ioUserNew.emit('updateUserLoc', clientMarkerUpdate);
}
...
// 	ioUserNew.emit('updateDriverLoc', "ok");
// }


console.log('User: Now you can type: \n');

console.log('For Stage 1: \n initUser, groupUpdate, forceDisconn, stopGroup');
